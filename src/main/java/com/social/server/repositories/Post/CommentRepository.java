...
public interface CommentRepository extends JpaRepository<Comments,String> {
    ...
    SELECT new com.social.server.dtos.CommentDTO(co.id,co.postId,co.content,co.imageUrl,co.owner, co.parentId,co.isDeleted,co.postedAt, co.updatedAt, (select count(*) from Comments c where c.parentId = co.id))  FROM Comments co  WHERE co.parentId = :parentId AND co.type = :type ORDER BY co.postedAt DESC
    """;
